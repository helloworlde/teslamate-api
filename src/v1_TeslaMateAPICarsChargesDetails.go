...
// TeslaMateAPICarsChargesDetailsV1 单次充电详情（含采样点）。
// @Summary 充电详情
// @Tags charges
// @Produce json
// @Param CarID path int true "车辆 ID" example(1)
// @Param ChargeID path int true "充电过程 ID"
// @Success 200 {object} RespChargeDetail
...
func TeslaMateAPICarsChargesDetailsV1(c *gin.Context) {
	...
	var (
		CarName                       NullString
		charge                        APIChargeDetail
		ChargeDetailsData             []APIChargeDetailRowFixed
	...
	for rows.Next() {
		...
		chargedetails := APIChargeDetailRowFixed{}
	...
	//
	// build the data-blob
	jsonData := RespChargeDetail{
		Data: RespChargeDetailData{
			Car: APICarRef{
				CarID:   CarID,
				CarName: CarName,
			},
