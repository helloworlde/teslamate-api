...
// TeslaMateAPICarsStatusV1 MQTT 缓存的实时车辆状态（对齐 Overview 等实时卡片）。
// @Summary 实时状态（MQTT）
// @Tags cars
// @Produce json
// @Param CarID path int true "车辆 ID" example(1)
// @Success 200 {object} MQTTStatusResponse
// @Failure 501 {object} RespAPIError
...
func (s *statusCache) TeslaMateAPICarsStatusV1(c *gin.Context) {
	if s.mqttDisabled {
		...
		TeslaMateAPIHandleOtherResponse(c, http.StatusNotImplemented, "TeslaMateAPICarsStatusV1", RespAPIError{Error: "mqtt disabled.. status not accessible!"})
		return
	}

	if !s.mqttConnected {
		log.Println("[notice] TeslaMateAPICarsStatusV1 mqtt is disconnected.. can not return status for car without mqtt!")
		TeslaMateAPIHandleOtherResponse(c, http.StatusInternalServerError, "TeslaMateAPICarsStatusV1", RespAPIError{Error: "mqtt disconnected.. status not accessible!"})
		return
	}
	...
	var (
		CarData                                      APICarRef
		MQTTInformationData                          MQTTStatusSnapshot
	...
	MQTTInformationData.CarGeodata.Location = MQTTStatusGeoPoint{Latitude: stat.MQTTDataLocation.Latitude, Longitude: stat.MQTTDataLocation.Longitude}
	...
	MQTTInformationData.DrivingDetails.ActiveRoute.Location = MQTTStatusGeoPoint{Latitude: stat.MQTTDataActiveRoute.Location.Latitude, Longitude: stat.MQTTDataActiveRoute.Location.Longitude}
	...
	//
	// build the data-blob
	jsonData := MQTTStatusResponse{
		Data: MQTTStatusPayload{
			Car:             CarData,
			MQTTInformation: MQTTInformationData,
			TeslaMateUnits: APIUnitsLengthTempPressure{
