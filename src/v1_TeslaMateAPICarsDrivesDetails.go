...
// TeslaMateAPICarsDrivesDetailsV1 单次行程详情（含轨迹点）。
// @Summary 行程详情
// @Tags drives
// @Produce json
// @Param CarID path int true "车辆 ID" example(1)
// @Param DriveID path int true "行程 ID"
// @Success 200 {object} RespDriveDetail
...
func TeslaMateAPICarsDrivesDetailsV1(c *gin.Context) {
	...
	var (
		CarName                       NullString
		drive                         APIDriveDetail
		DriveDetailsData              []APIDriveDetailPoint
	...
	for rows.Next() {
		...
		drivedetails := APIDriveDetailPoint{}
	...
	//
	// build the data-blob
	jsonData := RespDriveDetail{
		Data: RespDriveDetailData{
			Car: APICarRef{
				CarID:   CarID,
				CarName: CarName,
			},
